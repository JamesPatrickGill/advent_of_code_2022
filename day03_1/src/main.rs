...
fn main() {
    ...
    let mut pack_errors = vec![];
    for bag in bags.into_iter() {
        let size = bag.len();
        let pockets = bag.split_at(size / 2);

        let set: HashSet<char> = pockets.0.chars().collect();
        let set2: HashSet<char> = pockets.1.chars().collect();
        for item in set2.into_iter() {
            if set.contains(&item) {
                pack_errors.push(item)
            }
        }
    }
    score(pack_errors);
